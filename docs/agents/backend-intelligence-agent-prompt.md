...
- 当前阶段：Round-4 已收口，Phase 9 正在把 fast ingest / async analysis / queue-status 与主动短视频 evidence 增强并入主线
...
- 当前允许做：
  - 手动 6 秒短视频 evidence
  - 默认关闭、显式开启、受 queue-status / 冷却 / 上限保护的主动短视频 directive
- 当前明确不做：
  - 全天连续录像
  - 长视频自动切片或端侧环形 pre-buffer
  - 不可解释、不可关闭的语音开启视频
  - 微表情自动结论
